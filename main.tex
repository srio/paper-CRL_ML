...
\end{enumerate}
 ...
 % We show in  Fig~\ref{fig:v12v13profiles}a a few data and predicted profiles (calculated using the predicted Gram-Schmidt coefficients). 
...
\begin{figure}\label{fig:v12v13}
    ...
    % \includegraphics[width=0.99\textwidth]{figures/v14.png}
    \caption{Accuracy of the training data and validation data, for a) $N_S$=1000, and b) $N_S$=5000.
    % , c) $N_S$=5000 but using Zernike coefficients instead of Gram-Schmidt.
    }
\end{figure}
...
We have shown in this work that a CNN trained with synthetic data is able to retrieve the profile error in an x-ray lens giving as input a set of intensity distributions measured or calculates at different positions. The trained CNN is a robust model that works satisfactorly in many conditions. Many tuning parameters that can be changed in the CNN and also physical phenomena like the number of planes used or the effect of partial coherence have been analyzed and, although showing more or less sensitivity to the accuracy of the results, they always produce reasonaly good results. This feasibility study opens the way to other more complete analyses. Next computational effort will be addressed, using 2D images, which will certainly increase the amount of data and learning process, but there is no reason for not getting similar good results. The usefulness of the trained CNN using experimental images will then be addressed in a future work.  

% \begin{itemize}
%     \item What happens if we make a larger interval for sampling the Zernike coeffs? And if we put uniform distribution for all?  
%     \item add ``experimental" features to the data: noise, dynamic range.
%     \item Any thought for a possible experimental validation? Transport the problem to the mirror - ID09 measurements?  
%     \item Once optimized the 1D cases, plan 2D simulations with SRW (and COMSYL?)
%     \item investigate sklearn NN regression\footnote{https://scikit-learn.org/stable/modules/neural\_networks\_supervised.html : \\ MLPRegressor also supports multi-output regression, in which a sample can have more than one target.}
%     \item Using a pretrained convnet. 
%     Test over other lens configurations.
%     \item Can we create an "image embedded" from the pretrained convnet? Is portable to sklearn? Use with Orange?    
% \end{itemize}
